...
namespace Com.Bigdata.Dis.Sdk.DISCommon.Signer
{
    ...
    /// </summary>
    public class DISSignerEx : AbstractDISSigner
    {
        ...
        public const string SDK_TERMINATOR = "sdk_request";
        public static readonly byte[] SDK_TERMINATORBytes = Encoding.UTF8.GetBytes(SDK_TERMINATOR);
        ...
        public DISSignerEx(string serviceName, string regionName)
        {
        ...
        /// </param>
        /// <exception cref="OBS.Runtime.SignatureException">
        ...
        public override void Sign(IRequest request, ICredentials credentials)
        {
            ...
            string sha256Value;
            if (request.Headers.TryGetValue("x-sdk-content-sha256", out sha256Value))
            {
                if ("required".Equals(sha256Value))
                {
            ...
            request.Headers.Add("Authorization", this.BuildAuthorizationHeader(request, signature, sanitizedCredentials, signerParams));
            //var metrics = new RequestMetrics();
            //SignHttp(request, metrics, sanitizedCredentials.GetAccessKeyId(), sanitizedCredentials.GetSecretKey(), signerParams);
        }

        private string BuildAuthorizationHeader(IRequest request, byte[] signature, ICredentials credentials, SignerRequestParamsEx signerParams)
        {
            ...
            string signerHeaders = "SignedHeaders=" + canonicalizedHeaderNames;
        ...
        protected byte[] ComputeSignature(string stringToSign, byte[] signingKey, SignerRequestParamsEx signerRequestParams)
        {
            var signature = ComputeKeyedHash(SignerAlgorithm, signingKey, stringToSign);

            return signature;
        }

        private byte[] DeriveSigningKey(ICredentials credentials, SignerRequestParamsEx signerRequestParams)
        {
            var key = ComposeSigningKey(credentials.GetSecretKey(),
                                        this.regionName,
                                        signerRequestParams.GetFormattedSigningDate(),
                                        //ConfigurationManager.AppSettings["CurrentTimeShort"].ToString(),
                                        this.serviceName);

            return key;
        }

        protected string CreateStringToSign(string canonicalRequest, SignerRequestParamsEx signerParams)
        {
            ...
            stringToSignBuilder.Append("\n")
                               ...
                               //.Append(ConfigurationManager.AppSettings["CurrentTime"].ToString())
                               .Append("\n")
                               .Append(signerParams.GetScope())
                               .Append("\n")
                               .Append(BinaryUtils.ToHex(ComputeHash(canonicalRequest)));

            string stringToSign = stringToSignBuilder.ToString();
            return stringToSign;
        }

        protected string CreateCanonicalRequest(IRequest request, string contentSha256)
        {

            //string sd = Encoding.UTF8.GetString(request.Content);
        ...
        {
            //char[] ksecret = null;
            byte[] ksecret = new byte[] { };
            try
            {
                //ksecret = (Scheme + OBS.S3.AesCryptor.Decrypt(awsSecretAccessKey)).ToCharArray();
                ksecret = Encoding.UTF8.GetBytes(SDK_NAME + awsSecretAccessKey);
                //var hashDate = ComputeKeyedHash(SignerAlgorithm, Encoding.UTF8.GetBytes(ksecret), Encoding.UTF8.GetBytes(date));
                var hashDate = ComputeKeyedHash(SignerAlgorithm, ksecret, Encoding.UTF8.GetBytes(date));
                ...
                return ComputeKeyedHash(SignerAlgorithm, hashService, SDK_TERMINATORBytes);
                                                      ...
                                                      IRequest request)
        {
            ...
            //2015-7-30 region not to lower
            if (!string.IsNullOrEmpty(authenticationRegion))
                return authenticationRegion;
    ...
    internal class AWS4PreSignedUrlSigner : AWS4Signer
    {
                                                 ...
                                                 string overrideSigningRegion)
        {
            ...
            var region = overrideSigningRegion ?? clientConfig.AuthenticationRegion;
