...
{
    /// <summary>
    /// Parameters for uploading to Obs S3 a file using HTTP POS
    /// </summary>
    /// <remarks>
    /// <para>
    ...
    public class S3PostUploadRequest
    {
        ...
        /// <summary>
        /// Specifies an Obs S3 access control list
        /// </summary>
        ...
        /// </para><para>
        /// If the value is set to OK or NoContent, Obs S3 returns an empty document with a 200 or 204 status code.
        /// </para><para>
        /// If the value is set to Created, Obs S3 returns an XML document with a 201 status code.
        /// </para><para>
        /// If the value is not set or if it is set to an invalid value, Obs S3 returns an empty document with a 204 status code. 
        /// </para>
        /// </remarks>
        ...
        internal void WriteFormData(string boundary, Stream outputStream)
        {
            ...
            if (String.IsNullOrEmpty(this.ContentType))
            {
                if (this.Key.IndexOf('.') > -1)
                    this.ContentType = ObsS3Util.MimeTypeFromExtension(this.Key.Substring(this.Key.LastIndexOf('.')));
                else if (!String.IsNullOrEmpty(this.Path) && this.Path.IndexOf('.') > -1)
                    this.ContentType = ObsS3Util.MimeTypeFromExtension(this.Key.Substring(this.Path.LastIndexOf('.')));
            }
