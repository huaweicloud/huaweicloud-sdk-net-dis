...
namespace OBS.S3.Model.Internal.MarshallTransformations
{
    ...
    internal class GetBucketLocationResponseUnmarshaller : S3ReponseUnmarshaller
    {
            ...
            while (context.Read())
            {
                if (context.IsStartElement || context.IsAttribute)
                {
                    if (context.TestExpression("LocationConstraint", targetDepth))
                    {
                        ...
                        //2015-5-27 w00322557
                        LoggerMgr.Log_Run_Info(OBS.S3.ObsClient.strProduct,string.Format("GetBucketLocationResponseUnmarshaller Location: {0}", response.Location.Value));
                        continue;
                    }
                }
        ...
        public override ObsServiceException UnmarshallException(XmlUnmarshallerContext context, Exception innerException, HttpStatusCode statusCode)
        {
            ...
            //2015-5-27 w00322557
            LoggerMgr.Log_Run_Error(OBS.S3.ObsClient.strProduct,string.Format("GetBucketLocationResponseUnmarshaller HttpStatus: {0}", statusCode.ToString()));
            LoggerMgr.Log_Run_Error(OBS.S3.ObsClient.strProduct,string.Format("GetBucketLocationResponseUnmarshaller exception: {0}", innerException.Message));
