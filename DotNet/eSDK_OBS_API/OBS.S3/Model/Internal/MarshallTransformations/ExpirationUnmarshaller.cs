            ...
            while (context.Read())
            {
                if (context.IsStartElement || context.IsAttribute)
                {
                    if (context.TestExpression("Date", targetDepth))
                    {
                        ...
                        LoggerMgr.Log_Run_Info(OBS.S3.ObsClient.strProduct,string.Format("LifecycleRuleExpiration Date: {0}", expiration.Date.ToShortDateString()));
                        continue;
                    }
                    if (context.TestExpression("Days", targetDepth))
                    {
                        expiration.Days = IntUnmarshaller.GetInstance().Unmarshall(context);
                        LoggerMgr.Log_Run_Info(OBS.S3.ObsClient.strProduct,string.Format("LifecycleRuleExpiration Days: {0}", expiration.Days));
                        continue;
                    }
                }
