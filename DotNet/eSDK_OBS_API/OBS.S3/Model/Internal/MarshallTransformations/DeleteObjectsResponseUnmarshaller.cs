...
namespace OBS.S3.Model.Internal.MarshallTransformations
{
    ...
    internal class DeleteObjectsResponseUnmarshaller : S3ReponseUnmarshaller
    {
            ...
            while (context.Read())
            {
                if (context.IsStartElement || context.IsAttribute)
                {
                    if (context.TestExpression("Deleted", targetDepth))
                    {
                        ...
                        //2015-5-27 w00322557
                        LoggerMgr.Log_Run_Info(OBS.S3.ObsClient.strProduct,string.Format("DeleteObjectsResponseUnmarshaller Deleted."));
                        continue;
                    }
                    if (context.TestExpression("Error", targetDepth))
                    {
                        response.DeleteErrors.Add(ErrorsItemUnmarshaller.Instance.Unmarshall(context));
                        LoggerMgr.Log_Run_Info(OBS.S3.ObsClient.strProduct,string.Format("DeleteObjectsResponseUnmarshaller Error."));
                        continue;
                    }
                }
        ...
        public override ObsServiceException UnmarshallException(XmlUnmarshallerContext context, Exception innerException, HttpStatusCode statusCode)
        {
            ...
            //2015-5-27 w00322557
            LoggerMgr.Log_Run_Error(OBS.S3.ObsClient.strProduct,string.Format("DeleteObjectsResponseUnmarshaller HttpStatus: {0}", statusCode.ToString()));
            LoggerMgr.Log_Run_Error(OBS.S3.ObsClient.strProduct,string.Format("DeleteObjectsResponseUnmarshaller exception: {0}", innerException.Message));
