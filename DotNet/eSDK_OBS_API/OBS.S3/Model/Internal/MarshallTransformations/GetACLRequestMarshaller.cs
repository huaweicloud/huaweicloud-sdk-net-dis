...
namespace OBS.S3.Model.Internal.MarshallTransformations
{
    ...
    /// </summary>       
    public class GetACLRequestMarshaller : IMarshaller<IRequest, GetACLRequest> ,IMarshaller<IRequest,OBS.Runtime.ObsWebServiceRequest>
	{
        ...
        {
            IRequest request = new DefaultRequest(getObjectAclRequest, "ObsS3");

            request.HttpMethod = "GET";
            //2015-5-27 w00322557
            LoggerMgr.Log_Run_Info(OBS.S3.ObsClient.strProduct,string.Format("GetACLRequestMarshaller HttpMethod: {0}", request.HttpMethod));

            request.ResourcePath = string.Format(CultureInfo.InvariantCulture, "/{0}/{1}",
                                                 S3Transforms.ToStringValue(getObjectAclRequest.BucketName),
                                                 S3Transforms.ToStringValue(getObjectAclRequest.Key));
            LoggerMgr.Log_Run_Info(OBS.S3.ObsClient.strProduct,string.Format("GetACLRequestMarshaller ResourcePath: {0}", request.ResourcePath));
