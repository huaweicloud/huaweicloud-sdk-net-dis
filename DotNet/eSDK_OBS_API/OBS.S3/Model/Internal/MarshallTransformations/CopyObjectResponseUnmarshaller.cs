...
namespace OBS.S3.Model.Internal.MarshallTransformations
{
    ...
    internal class CopyObjectResponseUnmarshaller : S3ReponseUnmarshaller
    {
            ...
            while (context.Read())
            {
                if (context.IsStartElement || context.IsAttribute)
                {
                    if (context.TestExpression("ETag", targetDepth))
                    {
                        ...
                        LoggerMgr.Log_Run_Info(OBS.S3.ObsClient.strProduct,string.Format("CopyObjectResponseUnmarshaller ETag: {0}", response.ETag));
                        continue;
                    }
                    if (context.TestExpression("LastModified", targetDepth))
                    {
                        response.LastModified = StringUnmarshaller.GetInstance().Unmarshall(context);
                        LoggerMgr.Log_Run_Info(OBS.S3.ObsClient.strProduct,string.Format("CopyObjectResponseUnmarshaller LastModified: {0}", response.LastModified));
                        continue;
                    }
                }
        ...
        public override ObsServiceException UnmarshallException(XmlUnmarshallerContext context, Exception innerException, HttpStatusCode statusCode)
        {
            ...
            //2015-5-27 w00322557
            LoggerMgr.Log_Run_Error(OBS.S3.ObsClient.strProduct,string.Format("CopyObjectResponseUnmarshaller HttpStatus: {0}", statusCode.ToString()));
            LoggerMgr.Log_Run_Error(OBS.S3.ObsClient.strProduct,string.Format("CopyObjectResponseUnmarshaller exception: {0}", innerException.Message));
