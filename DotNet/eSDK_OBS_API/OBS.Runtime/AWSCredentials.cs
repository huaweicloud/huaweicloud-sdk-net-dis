...
namespace OBS.Runtime
{
    ...
    /// To manage the SDK Store with the SDK use OBS.Util.ProfileManager. If the profile is not found in the SDK Store then credentials file shared with other AWS SDKs 
    ...
    public class StoredProfileAWSCredentials : AWSCredentials
    {
        ...
        public StoredProfileAWSCredentials()
            : this(ObsConfigs.OBSProfileName)
        {
        ...
        public StoredProfileAWSCredentials(string profileName)
            : this(profileName, ObsConfigs.OBSProfilesLocation)
        {
        ...
        public StoredProfileAWSCredentials(string profileName, string profilesLocation)
        {
            ...
            if (string.IsNullOrEmpty(profilesLocation))
            {
                ...
                if (OBS.Util.ProfileManager.TryGetAWSCredentials(lookupName, out credentials))
                {
    ...
    public abstract class RefreshingAWSCredentials : AWSCredentials
    {
        ...
        private void UpdateToGeneratedCredentials(CredentialsRefreshState state)
        {
            ...
            if (ShouldUpdate)
            {
                throw new ObsClientException("The retrieved credentials have already expired");
            }
    ...
    public class InstanceProfileAWSCredentials : RefreshingAWSCredentials
    {
        ...
        protected override CredentialsRefreshState GenerateNewCredentials()
        {
            ...
            // Return credentials that will expire in at Obst one hour
        ...
        private CredentialsRefreshState GetRefreshState()
        {
            ...
            {
                throw new ObsServiceException(string.Format(CultureInfo.InvariantCulture, 
        ...
        private static string GetContents(Uri uri)
        {
            try
            {
                ...
                if (request == null)
                {
                    throw new ObsServiceException("Create http request error");
                }
                ...
                using (HttpWebResponse response = request.EndGetResponse(asyncResult) as HttpWebResponse)
                {
                    if (response == null)
                    {
                        throw new ObsServiceException("Get http response error");
                    }
            ...
            {
                throw new ObsServiceException("Unable to reach credentials server");
            }
        }
        ...
        internal static AWSCredentials GetCredentials(bool fallbackToAnonymous = false)
        {
            ...
            if (cachedCredentials == null)
            {
                ...
                using (StringWriter writer = new StringWriter(CultureInfo.InvariantCulture))
                {
                    ...
                    throw new ObsServiceException(writer.ToString());
                }
            }
