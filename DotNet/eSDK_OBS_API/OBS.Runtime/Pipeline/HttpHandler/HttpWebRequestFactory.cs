...
namespace OBS.Runtime.Internal
{
    ...
    public class HttpRequest : IHttpRequest<Stream>
    {
        ...
        private static bool _isInstallCertificat = false;
        ...
        public virtual void ConfigureRequest(IRequestContext requestContext)
        {
            ...
            // HTTPS, validate the certificate
            if (clientConfig.ValidateCertificate == true)
            {
            ...
            else
            {
                ...
                ServicePointManager.ServerCertificateValidationCallback = delegate(
                                                                        ...
                                                                        SslPolicyErrors errors)
                                                                            {
                                                                                // Add to fix bug first time seed is low
        ...
        private static void InstallCertificate(X509Certificate certificate)
        {
              ...
              SslPolicyErrors sslPolicyErrors)
        {
            ...
            // None, validate successfully
            if (sslPolicyErrors == SslPolicyErrors.None)
            {
                LoggerMgr.Log_Run_Info(OBS.S3.ObsClient.strProduct, "Validate server certificate successfully.");
                return true;
            }
            else
            {
                        ...
                        sslPolicyErrors) == SslPolicyErrors.RemoteCertificateChainErrors)
                {
                    ...
                    foreach (X509ChainStatus status in chain.ChainStatus)
                    {
                        ...
                        strStatus += string.Format("status code ={0}; ", status.Status);
                        strStatus += string.Format("Status info ={0} \n",  status.StatusInformation);
                    }
                    msg = string.Format("Certificate Chain Error \n{0}", strStatus);
                }
                msg = "validate certificate failed, " + msg;
                LoggerMgr.Log_Run_Error(OBS.S3.ObsClient.strProduct, msg);
               return false;
            }
