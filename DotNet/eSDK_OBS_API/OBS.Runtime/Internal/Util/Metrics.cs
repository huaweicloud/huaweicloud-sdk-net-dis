...
namespace OBS.Runtime.Internal.Util
{
    public class RequestMetrics : IRequestMetrics
    {
        ...
        public override string ToString()
        {
            ...
            if (ObsConfigs.LoggingConfig.LogMetricsCustomFormatter != null)
            {
                try
                {
                    lock(metricsLock)
                    {
                        builder.Append(ObsConfigs.LoggingConfig.LogMetricsCustomFormatter.FormatMetrics(this));
                    }
            ...
            if (ObsConfigs.LoggingConfig.LogMetricsFormat == LogMetricsFormatOption.JSON)
            {
                lock (metricsLock)
                {
        ...
        public string ToJSON()
        {
            ...
            {
                jw.WritePropertyName(kvp.Key.ToString());
                List<OBS.Runtime.IMetricsTiming> timings = kvp.Value;
                if (timings.Count > 1)
                    jw.WriteArrayStart();
