/*******************************************************************************
 *  Copyright 2008-2012 OBS.com, Inc. or its affiliates. All Rights Reserved.
...
namespace OBS
{
    ...
    {
        //2015-5-13 w00322557
        const string REGIONS_FILE = "eSDK_OBS_API.endpoints.xml";
        ...
        {
            var endpointsPath = ObsConfigs.EndpointDefinition;
        ...
        {
            Assembly assembly = OBS.Util.TypeFactory.GetTypeInfo(typeof(RegionEndpoint)).Assembly;
        ...
        static bool TryLoadEndpointDefinitionsFromAssemblyDir()
        {
            ...
            try
            {
                var assembly = typeof(OBS.RegionEndpoint).Assembly;
        ...
        static void LoadEndpointDefinitionFromFilePath(string path)
        {
            if (!System.IO.File.Exists(path))
                throw new ObsServiceException(string.Format(CultureInfo.InvariantCulture, "Local endpoint configuration file {0} override was not found.", path));
            ...
            while (retries < MAX_DOWNLOAD_RETRIES)
            {
                ...
                catch (WebException e)
                {
                    ...
                    if (retries == MAX_DOWNLOAD_RETRIES)
                        throw new ObsServiceException(string.Format(CultureInfo.InvariantCulture, "Error downloading regions definition file from {0}.", url), e);
                }
